...
class MagnetLoss(nn.Module):
    ...
    def forward(self, r, classes, m, d, alpha=1.0):
        ...
        self.cluster_classes = self.classes[0:m*d:d] #The generated batch will consist of m clusters each with d consecutive examples
        self.n_clusters = m
        self.alpha = alpha
        ...
        intra_cluster_mask = Variable(comparison_mask(clusters_tensor, n_clusters_tensor).type(GPU_FLOAT_DTYPE)) # picks examples within the same cluster
        ...
        # Compute denominator
        diff_class_mask = Variable(comparison_mask(classes_tensor, cluster_classes_tensor).type(GPU_FLOAT_DTYPE)) # picks examples in OTHER CLASSES
